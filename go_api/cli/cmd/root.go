...
Bash:
  ...
  # macOS:
  # If you use bash on macOS, you need Homebrew to install bash-completion@2
  # and a modern bash version:
  # $ brew install bash bash-completion@2
  # Then add bash-completion to your ~/.bash_profile:
  $ echo '[[ -r "$(brew --prefix)/etc/profile.d/bash_completion.sh" ]] && . "$(brew --prefix)/etc/profile.d/bash_completion.sh"' >> ~/.bash_profile
  # And switch to the new shell (this may require your password):
  $ echo $(brew --prefix)/bin/bash | sudo tee -a /etc/shells
  $ chsh -s $(brew --prefix)/bin/bash
  # After restarting your terminal, install completion:
  $ lit completion bash > $(brew --prefix)/etc/bash_completion.d/lit
...
func init() {
	...
	runCmd.Flags().StringP("input_prompt_file", "f", "", "Path to a file containing the input prompt for single-turn inference")
