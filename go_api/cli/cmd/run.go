...
var runCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		var fileName string
		info, isRegistryAlias := modelRegistry[modelName]
		if isRegistryAlias {
		...
		if _, err := os.Stat(modelPath); os.IsNotExist(err) {
			...
			if !isRegistryAlias {
				...
				for alias, regInfo := range modelRegistry {
					if path.Base(regInfo.url) == fileName {
						pullHint = alias
						break
					}
				}
		...
		inputPromptFile, err := cmd.Flags().GetString("input_prompt_file")
		if err != nil {
			return fmt.Errorf("failed to get 'input_prompt_file' flag: %w", err)
		}
		...
		if inputPromptFile != "" {
			inputBytes, err := os.ReadFile(inputPromptFile)
			if err != nil {
				return fmt.Errorf("failed to read input prompt file %q: %w", inputPromptFile, err)
			}
			...
			for msg := range stream {
				if msg.Err != nil {
					return fmt.Errorf("stream error: %v", msg.Err)
				}
			...
			if benchmark {
				if err := printBenchmarkInfo(session); err != nil {
					return err
				}
			}
			return nil
		...
		for {
			...
			if strings.HasPrefix(input, "/") {
				...
				case "/clear":
					session.Delete()
					newSession, err := eng.CreateSession()
					if err != nil {
						return fmt.Errorf("failed to create new session: %w", err)
					}
					session = newSession
					fmt.Println("Session context cleared.")
				case "/show":
					fmt.Printf("Model: %s\n", modelName)
...
func printBenchmarkInfo(session *engine.Session) error {
	info, err := session.BenchmarkInfo()
	if err != nil {
		return fmt.Errorf("error getting benchmark info: %v", err)
	}
	fmt.Println("---")
