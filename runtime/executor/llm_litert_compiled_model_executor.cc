...
absl::Status LlmLiteRtCompiledModelExecutorBase::SetCurrentStep(int new_step) {
  ...
  int processed_tokens_count = processed_tokens->TokenCount();
  RET_CHECK_LE(new_step, processed_tokens_count)
          .SetCode(absl::StatusCode::kInvalidArgument)
      << "New step cannot be greater than the number of processed tokens: "
      << processed_tokens_count;
  RET_CHECK_GE(new_step, 0).SetCode(absl::StatusCode::kInvalidArgument)
      << "New step cannot be negative.";
  if (new_step == processed_tokens_count) {
